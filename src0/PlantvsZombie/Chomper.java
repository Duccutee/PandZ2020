		...
		if(!swallow) {
			for(int i=0; i<zombies.size(); i++) {
				...
				//if(tempZombie.getBound().intersects(this.getBound())) {
					if( this.getLane() == tempZombie.getLane() && (tempZombie.getX() - this.getX() ) <=95*2 ) {
					eat=true;
...
public class Chomper extends Vegan {
	...
	public void paint(Graphics g) {
		...
		g.drawImage(getChomperImagee(), x,y, null);
		checkCollideZombie();}
	...
	public void checkCollideZombie() {
		...
		if(!swallow) {
			for(int i=0; i<zombies.size(); i++) {
				Zombie tempZombie = zombies.get(i);
				if(this.getLane() == tempZombie.getLane()) System.out.println(tempZombie.getX() - this.getX());
